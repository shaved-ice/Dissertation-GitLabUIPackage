...
public class MainScript : MonoBehaviour
{
    ...
    private GitLabUI.AutonomousDeleteExtra deleter;
    private GitLabUI.AutonomousReplaceMissing replacer;
    // -- --
    ...
    private void checkWin() 
    {
        ...
        while (deleter.filepathsDeleted.Count > 0 && currentLife < 3)
        {
            deleter.filepathsDeleted.Remove(deleter.filepathsDeleted[0]); //remove the first file from the list
        ...
        while (replacer.filepathsReplaced.Count > 0 && currentLife < 3)
        {
            replacer.filepathsReplaced.Remove(replacer.filepathsReplaced[0]);
            livesLostText = livesLostText + "You deleted something! You've lost a life.\n"; //add a mistake message and lose a life
            Lives[currentLife].SetActive(false);
            currentLife = currentLife + 1;
            if (currentLife == 3){ break;} //we can stop if we've already lost
        }
        while (replacer.filepathsUpdated.Count > 0 && currentLife < 3 && !win)
        {
            if (replacer.filepathsUpdated[0] != targetPath){
                replacer.filepathsUpdated.Remove(replacer.filepathsUpdated[0]);
    ...
    private void addTarget() 
    {
        ...
        deleter = package.createAutonomousDelete(mainURL, projectCode, token, package, repoSaveState, "removing extra files" ,"main", "HEAD", 0); //start the delete auto
        replacer = package.createAutonomousReplace(mainURL, projectCode, token, package, repoSaveState, "replacing previous files", "fixing file contents", "main", "HEAD", 0); //start the replace auto
